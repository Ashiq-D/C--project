...
const companies = [
  {
    ...
    icon: (
      <svg width="28" height="28" viewBox="0 0 24 24" fill="none" stroke="#ff4b3e" strokeWidth="1.6" strokeLinecap="round" strokeLinejoin="round">
  ...
  useEffect(() => {
    ...
    const draw = () => {
      ...
      for (const p of particles) {
        ...
        ctx.fillStyle = Math.random() > 0.5 ? `rgba(255, 75, 62, ${p.alpha})` : `rgba(212, 169, 58, ${p.alpha})`;
        ctx.fill();
      }
      ...
      for (let i = 0; i < particles.length; i++) {
        for (let j = i + 1; j < particles.length; j++) {
          ...
          if (dist < 120) {
            ...
            ctx.strokeStyle = `rgba(255, 75, 62, ${0.05 * (1 - dist / 120)})`;
            ctx.lineWidth = 0.5;
            ctx.stroke();
          }
        }
...
function AccordionItem({
  ...
  return (
    <motion.div
      ...
      style={{
        background: isOpen ? "rgba(17, 15, 36, 0.75)" : "rgba(17, 15, 36, 0.5)",
        border: `1px solid ${isOpen ? "rgba(255,75,62,0.5)" : "rgba(212,169,58,0.2)"}`,
        boxShadow: isOpen ? "0 8px 32px rgba(0,0,0,0.3), inset 0 1px 0 rgba(255,255,255,0.05)" : "0 4px 16px rgba(0,0,0,0.2), inset 0 1px 0 rgba(255,255,255,0.03)",
    ...
    >
      ...
      <div
        ...
        style={{
          backgroundImage: "linear-gradient(90deg, transparent, #ff4b3e, transparent)",
          opacity: isOpen ? 1 : 0,
        }}
      />
      ...
      >
        ...
        <div
          ...
          style={{
            background: isOpen ? "rgba(255,75,62,0.15)" : "rgba(255,75,62,0.08)",
            border: `1px solid ${isOpen ? "rgba(255,75,62,0.35)" : "rgba(255,75,62,0.15)"}`,
            transition: "all 0.3s",
          }}
        >
          {company.icon}
        ...
        <div className="flex-1 min-w-0">
          ...
          <p className="text-techmak-gold/70 text-xs font-medium tracking-wider uppercase mt-0.5">
            {company.role}
          </p>
        </div>
        ...
        {company.website ? (
          <motion.a
            ...
            rel="noopener noreferrer"
            onClick={(e) => e.stopPropagation()}
            whileHover={{ scale: 1.05, backgroundColor: "rgba(255, 75, 62, 0.2)" }}
            whileTap={{ scale: 0.95 }}
            className="hidden sm:inline-flex items-center px-4 py-1.5 rounded-full text-[10px] font-bold tracking-wider uppercase text-techmak-gold border border-techmak-gold/40 bg-techmak-gold/15 shrink-0 whitespace-nowrap shadow-[0_0_15px_rgba(212,169,58,0.1)] transition-all duration-300"
          >
            Visit Website
          </motion.a>
        ) : (
          <div className="hidden sm:inline-flex items-center px-3 py-1 rounded-full text-[10px] font-bold tracking-wider uppercase text-techmak-gold border border-techmak-gold/25 bg-techmak-gold/10 shrink-0 whitespace-nowrap">
            {company.category}
          </div>
        )}
          ...
          >
            <div className="px-6 pb-6 md:px-8 md:pb-8 pt-0">
              ...
              <div
                ...
                style={{
                  height: "1px",
                  backgroundImage: "linear-gradient(90deg, transparent, rgba(255,75,62,0.25), transparent)",
                }}
              />

              {/* Mobile category badge */}
              <div className="sm:hidden mb-4">
                <span className="inline-flex items-center px-3 py-1 rounded-full text-[10px] font-bold tracking-wider uppercase text-techmak-gold border border-techmak-gold/25 bg-techmak-gold/10">
                  {company.category}
                </span>
              </div>
              ...
              <div className="grid md:grid-cols-2 gap-6 md:gap-8">
                ...
                >
                  <p className="text-xs font-semibold uppercase tracking-widest text-techmak-gold/60 mb-3">
                    About
                  </p>
                    ...
                    {company.capabilities.map((cap, idx) => (
                      ...
                      >
                        <span className="shrink-0 mt-1.5">
                          <svg width="6" height="6" viewBox="0 0 24 24" fill="#ff4b3e">
...
export default function Brands() {
  ...
  return (
    <section id="brands" className="relative overflow-hidden pt-6 pb-24 md:pt-8 h-fit z-10">
      {/* ── Ambient background ── */}
      <FloatingParticles />
      <div className="pointer-events-none absolute -top-40 left-1/4 w-[600px] h-[600px] rounded-full bg-techmak-red/[0.04] blur-[150px]" />
      <div className="pointer-events-none absolute bottom-1/4 right-0 w-[500px] h-[500px] rounded-full bg-techmak-gold/[0.03] blur-[120px]" />
